...
function demoCourses(lang) {
  const names = [
    // Long, catalogue-style titles (3 lines on a card) so the 3-line clamp
    // and ellipsis are exercised by the demo data.
    [
      "Advanced Strategic Management, Leadership & Organisational Excellence for Senior Executives",
      "الإدارة الاستراتيجية المتقدمة والقيادة والتميز المؤسسي للمديرين التنفيذيين وكبار القادة",
    ...
    [
      "Digital Marketing Fundamentals: Social Media, SEO, Content Strategy & Campaign Analytics",
      "أساسيات التسويق الرقمي: وسائل التواصل الاجتماعي وتحسين محركات البحث واستراتيجية المحتوى وتحليل الحملات",
      "digital-marketing",
      "Marketing",
      "التسويق",
      1200,
    ...
    [
      "Project Management Professional (PMP) Exam Preparation & Practical Project Delivery Skills",
      "التحضير لاختبار إدارة المشاريع الاحترافية (PMP) ومهارات التنفيذ العملي للمشاريع",
      "pmp-preparation",
      "Management",
      "الإدارة",
      1800,
    ...
    [
      "Data Analysis, Reporting & Business Intelligence with Microsoft Excel and Power BI",
      "تحليل البيانات وإعداد التقارير وذكاء الأعمال باستخدام Microsoft Excel و Power BI",
      "data-analysis",
      "Technology",
      "التقنية",
      1350,
    ...
    [
      "Human Resources Management, Talent Acquisition, Performance Appraisal & Employee Relations",
      "إدارة الموارد البشرية واستقطاب المواهب وتقييم الأداء وعلاقات الموظفين في المؤسسات الحديثة",
      "hr-management",
      "Management",
      "الإدارة",
      1250,
    ...
    [
      "Public Relations, Corporate Communication, Media Handling & Reputation Management Strategies",
      "العلاقات العامة والاتصال المؤسسي والتعامل مع وسائل الإعلام واستراتيجيات إدارة السمعة",
      "public-relations",
      "Media",
      "الإعلام",
      1100,
  ...
  return names.map(([en, ar, slug, catEn, catAr, price], i) => ({
    ...
    // Specialisation drawn from the course's own category list so ids line up
    // with the sidebar options (demoSpecializations ids = categoryId*100+n).
    specialization: (() => { const list = demoSpecializations(lang, (i % 3) + 1); const sp = list[i % list.length]; return { id: sp.id, name: sp.name }; })(),
    // Percent off; 0 = no discount (drives the "discounted" filter).
    discount: [0, 15, 0, 20, 10, 0][i % 6],
    created_at: "2026-08-01T09:00:00.000000Z",
    language: lang === "ar" ? "ar" : "en",
    duration: t(lang, "1 week", "أسبوع واحد"),
    // First four demo venues (ids 1-4) so the city filter options match.
    city: (() => { const c = demoCities(lang)[i % 4]; return { id: c.id, name: c.name }; })(),
    description: t(
      lang,
...
// Client-supplied city landmark icons (SVG) in /asstes/cities — one per
// training venue. The first six are the marquee venues shown on the homepage
// (CourseByCity slices 0-6); the rest follow alphabetically for /show_cities.
function demoCities(lang) {
  ...
  return cities.map(([en, ar, slug], i) => ({
    ...
    name: t(lang, en, ar),
    slug,
    image: `/asstes/cities/${slug}.svg`,
    icon: `/asstes/cities/${slug}.svg`,
    courses_count: 8 + (i % 12) * 2,
  ...
  switch (resource) {
    case "courses": {
      ...
      if (typeof param === "string" && param.includes("=")) {
        ...
        // Both spellings: the sidebar sends price_min/price_max, the drawer slider min_price/max_price.
        const priceMin = Number(qs.get("price_min") || qs.get("min_price")); const priceMax = Number(qs.get("price_max") || qs.get("max_price"));
        if (priceMin) courses = courses.filter((c) => Number(c.price) >= priceMin);
        if (priceMax) courses = courses.filter((c) => Number(c.price) <= priceMax);
        if (qs.get("discounted") === "1") courses = courses.filter((c) => (c.discount || 0) > 0);
      }
      return {
        success: true,
        data: {
          ...
          has_more: false,
          next_cursor: null,
          // Option lists for the sidebar filters (the real API returns these too).
          categories: demoCategories(lang).slice(0, 3),
          specializations: [1, 2, 3].flatMap((id) => demoSpecializations(lang, id)),
          cities: demoCities(lang).slice(0, 4),
        },
      };
    }
    case "course": {
