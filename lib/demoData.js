...
function demoSpecializations(lang, categoryId) {
  ...
  // different set of sub-specialisations, and only 6 at a time (matching
  // the previous demo's card count) rather than dumping all 46 on one page.
  const offset = ((categoryId - 1) * 2) % specs.length;
  const rotated = [...specs.slice(offset), ...specs.slice(0, offset)].slice(0, 6);
