...
function demoCities(lang) {
  const cities = [
    ["London", "لندن", "london", "/asstes/london.jpg"],
    ["Dubai", "دبي", "dubai", "/asstes/img.jpg"],
    ["Istanbul", "إسطنبول", "istanbul", "/asstes/const.jpg"],
    ["Paris", "باريس", "paris", "/asstes/details.jpg"],
    ["Kuala Lumpur", "كوالالمبور", "kuala-lumpur", "/asstes/featured.jpg"],
    ["Barcelona", "برشلونة", "barcelona", "/asstes/last.jpg"],
  ];
  return cities.map(([en, ar, slug, image], i) => ({
    id: i + 1,
    name: t(lang, en, ar),
    slug,
    image,
...
export function demoResponse(resource, language = "en", param) {
  ...
  switch (resource) {
    ...
    case "course": {
      const course =
        demoCourses(lang).find((c) => c.slug === param) || demoCourses(lang)[0];
